...
const UnocoinConfigView = new Lang.Class({
  Name: "UnocoinConfigView",
  Extends: ProviderConfigView,

  _init: function (configWidget, indicatorConfig) {
    this.parent(configWidget, indicatorConfig);
    this._addSelectCurrency((new ApiProvider.UnocoinApi()).currencies);
  },

  _setApiDefaults: function (config) {
    if (config.get('api') !== 'unocoin') {
      config.attributes = {
        api: 'unocoin',
        currency: 'INR',
        attribute: 'last'
...
Signals.addSignalMethods(UnocoinConfigView.prototype);

const CoinsecureConfigView = new Lang.Class({
  Name: "CoinsecureConfigView",
  Extends: ProviderConfigView,
...
Signals.addSignalMethods(CoinsecureConfigView.prototype);
...
const IndicatorConfigView = new Lang.Class({
  ...
  _selectApi: function (api) {
    ...
    let apiConfigViews = {
      ...
      unocoin:         function () new UnocoinConfigView(widget, config),
      coinsecure:         function () new CoinsecureConfigView(widget, config)
    };
